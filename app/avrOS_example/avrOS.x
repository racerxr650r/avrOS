/* Script for AvrOS on AVR128DA28 */
...
__DATA_REGION_ORIGIN__ = DEFINED(__DATA_REGION_ORIGIN__) ? __DATA_REGION_ORIGIN__ : 0x804000;
__TEXT_REGION_LENGTH__ = DEFINED(__TEXT_REGION_LENGTH__) ? __TEXT_REGION_LENGTH__ : 96K;
...
MEMORY
{
  ...
  text_window (rx) : ORIGIN = 0x18000, LENGTH = 32k
...
SECTIONS
{
  ...
  CLI_CMDS ADDR(.rodata) + SIZEOF (.rodata) :
  {
	__start_CLI_CMDS = . ;
	*(CLI_CMDS)
	__stop_CLI_CMDS = . ;
  ...
  TMR_TABLE ADDR(QUE_TABLE) + SIZEOF (QUE_TABLE) :
  {
	__start_TMR_TABLE = . ;
	*(TMR_TABLE)
	__stop_TMR_TABLE = . ;
